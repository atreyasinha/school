...
// Created by Atreya Sinha on 12/05/21.
...
program:
    ...
    std::set<std::string> used_temp;

    std::set_difference(Animation_code::used_blocklist.begin(),
                        Animation_code::used_blocklist.end(),
                        Animation_code::defined_blocklist.begin(),
                        Animation_code::defined_blocklist.end(),
                        std::inserter(used_temp, used_temp.end()));

    for (auto it = used_temp.begin(); it != used_temp.end(); it++)      Error::error(Error::NO_BODY_PROVIDED_FOR_FORWARD, (*it));

    std::set<std::string> temp;

    std::set_difference(Animation_code::declared_blocklist.begin(),
                        Animation_code::declared_blocklist.end(),
                        Animation_code::defined_blocklist.begin(),
                        Animation_code::defined_blocklist.end(),
    ...
    for (auto it = temp.begin(); it != temp.end(); it++)                scopemgr.erase(*it);
    }
    ;
            ...
            while (cur != NULL) {
                try {
                    ...
                    try {
                        ...
                        else if (param_type == ANIMATION_BLOCK) {
                            const Animation_code *ac = co->as_animation_block();
                            if ($1 != ac->get_parameter_type())     Error::error(Error::GAME_OBJECT_ANIMATION_BLOCK_PARAMETER_TYPE_ERROR, gpl_type_to_string($1), 
                                                                                    gpl_type_to_string(ac->get_parameter_type()));
                            ptr->write_attribute(cur->name, ac);
...
initialization_block:
    T_INITIALIZATION statement_block {
        ...
        e.add_handler(Window::INITIALIZE, $2);
    }
    ;
...
termination_block:
    T_TERMINATION statement_block {
        Event_manager& e = Event_manager::instance();
        e.add_handler(Window::TERMINATE, $2);
    }
    ;
...
forward_declaration:
    T_FORWARD T_ANIMATION T_ID T_LPAREN animation_parameter T_RPAREN {
        Scope_manager& scopemgr = Scope_manager::instance();
        if (scopemgr.lookup(*$3))   Error::error(Error::PREVIOUSLY_DECLARED_VARIABLE, *$3);  

        Animation_code *ptr = new Animation_code(*$3, $5);
        scopemgr.add_to_current_scope(make_shared<Symbol>(*$3, ptr));
...
animation_block:
    animation_declaration statement_block {
        ...
        if (scopemgr.lookup(*$1)) {
            scopemgr.lookup(*$1)->as_lvalue()->mutate($2);
...
animation_declaration:
    T_ANIMATION T_ID T_LPAREN object_type T_ID T_RPAREN {
        Scope_manager& scopemgr = Scope_manager::instance();
        std::shared_ptr<Symbol> sym = scopemgr.lookup(*$2);

        if (sym == nullptr) {
            Animation_code *ptr = new Animation_code(*$2, $4);
            scopemgr.add_to_current_scope(make_shared<Symbol>(*$2, ptr));  
            sym = scopemgr.lookup(*$2);
        } 
        
        if (sym->get_type() == ANIMATION_CODE) {
            std::shared_ptr<const Constant> c { sym->as_constant() };

            Gpl_type type = c->as_animation_block()->get_parameter_type();

            if ($4 == type) {
                sym->as_lvalue()->mutate(*$5);
                $$ = $2;
            } else      Error::error(Error::ANIMATION_PARAM_DOES_NOT_MATCH_FORWARD, gpl_type_to_string($4), gpl_type_to_string(type));
        ...
        if ($4 == RECTANGLE) {
            Rectangle *ptr = new Rectangle();
            scopemgr.add_to_current_scope(std::make_shared<Symbol>(*$5, ptr));
        } else if ($4 == CIRCLE) {
            Circle *ptr = new Circle();
            scopemgr.add_to_current_scope(std::make_shared<Symbol>(*$5, ptr));
        } else if ($4 == TRIANGLE) {
...
assign_statement:
    variable T_ASSIGN expression { 
        if (($1->type() == ANIMATION_BLOCK) || ($1->type() == ANIMATION_CODE)) { 
            if ($3->type() != ANIMATION_BLOCK && $3->type() != ANIMATION_CODE) {
                Error::error(Error::ASSIGNMENT_TYPE_ERROR, gpl_type_to_string($1->type()), gpl_type_to_string($3->type()));
            } else if ($1->evaluate()->as_animation_block()->get_parameter_type() != $3->evaluate()->as_animation_block()->get_parameter_type()) {
                Error::error(Error::ANIMATION_BLOCK_ASSIGNMENT_PARAMETER_TYPE_ERROR, gpl_type_to_string($1->evaluate()->as_animation_block()->get_parameter_type()), gpl_type_to_string($3->evaluate()->as_animation_block()->get_parameter_type()));
            } 
            $$ = new Assign($1, $3);
        } else {
            if ($1->type() < $3->type()) {
            ...
            else if ($1->type() & (INT|DOUBLE|STRING))  $$ = new Assign($1, $3); 
            else {
    ...
    | variable T_PLUS_ASSIGN expression {   
        if ($1->type() & (INT|DOUBLE|STRING))                  $$ = new Plus_assign($1, $3); 
...
three:
    proto_expression
        | three T_TOUCHES proto_expression                          { $$ = Bin_op_check<Touches, TOUCHES>($1, $3, GAME_OBJECT ); }
        | three T_NEAR proto_expression                             { $$ = Bin_op_check<Near, NEAR>($1, $3, GAME_OBJECT ); }
        ;
